 ...
 * Supports creating new WBS forms and editing existing drafts via ?draft=<id>
 */
import { createFileRoute, Navigate, useNavigate, useSearch } from "@tanstack/react-router";
import { useEffect, useRef, useState } from "react";
import { toast } from "sonner";
import { useRoleContext } from "@/lib/role-context";
import { allClients, dhStore, getDraft, useDhStore, buildProjectDisplayId, buildWbsId } from "@/lib/dh-store";
...
function WbsNewProjectPage() {
  ...
  const { draft: draftId } = useSearch({ from: "/projects/new" });
  ...
  useEffect(() => {
    ...
    // Header
    if (draft.name && draft.name !== "Untitled Draft") setProjectName(draft.name);
    if (draft.contractType)      setContractType(draft.contractType);
    if (draft.engagementManager) setEngagementManager(draft.engagementManager);
    ...
    if (draft.clientId) {
      ...
      const c = clients.find((cl) => cl.id === draft.clientId);
      if (c) setClientSearch(c.name);
    }

    // Section A comments
    if (draft.sectionAComments) setSectionAComments(draft.sectionAComments);
    if (draft.sectionBComments) setSectionBComments(draft.sectionBComments);
    ...
    // Section B billing
    const acc = draft.wbsDetails?.accounts;
    if (acc) {
      if (acc.billingModel) setBillingModel(acc.billingModel);
      if (acc.paymentTerms) setPaymentTerms(acc.paymentTerms);
      if (acc.poStatus)     setPoStatus(acc.poStatus);
      if (acc.poNumber)     setPoNumber(acc.poNumber);
      if (acc.poDate)       setPoDate(acc.poDate);
      if (acc.targetDate)   setTargetDate(acc.targetDate);
      if (acc.contactName)  setContactName(acc.contactName);
      if (acc.contactNumber) setContactNumber(acc.contactNumber);
      if (acc.contactEmail) setContactEmail(acc.contactEmail);

      // Restore invoice rows
      if (acc.invoices?.length) {
        setInvoiceRows(
          acc.invoices.map((inv: any, i: number) => ({
            rowId: inv.id ?? `inv-${i}`,
    ...
    if (draft.wbsDetails?.services?.length) {
      const rows: ServiceRow[] = draft.wbsDetails.services.map((svc: any) => ({
        rowId: svc.id,
        taskId: svc.id,
        dept: svc.department,
        ...
        deliveryFormat: svc.finalDeliveryFormat ?? "PDF Report",
        tools: svc.tools ?? "",
        startDate: svc.startDate ?? todayIso,
        endDate: svc.endDate ?? todayIso,
        durationDays: svc.duration ?? svc.durationDays ?? 1,
        durationHrs: (svc.duration ?? svc.durationDays ?? 1) * 8,
        totalDays: svc.totalDays ?? svc.qty ?? 1,
        totalHrs: svc.totalHrs ?? (svc.qty ?? 1) * 8,
        unitPrice: svc.unitPrice ?? 0,
        total: svc.total ?? 0,
      }));
      setServiceRows(rows);

      // Rebuild selectedServices map so the picker reflects current state
      const sel: Record<string, Record<string, boolean>> = {};
      rows.forEach((r) => {
        if (!sel[r.dept]) sel[r.dept] = {};
        sel[r.dept][r.rowId] = true;
      });
  ...
  function handleSaveDraft() {
    // No mandatory fields — save whatever has been filled in so far
    const payload = {
      name: projectName.trim() || undefined,
      clientId: selectedClientId || undefined,
      description: sectionAComments || undefined,
      budget: subtotal || undefined,
      wbsStatus: "draft" as const,
      wbsSubStatus: "Draft",
      engagementManager: engagementManager || undefined,
      salesPerson: salesPerson || undefined,
      contractType: contractType || undefined,
      projectType: projectType || undefined,
      projectIssuedDate: projectIssuedDate || undefined,
      currency: currency || undefined,
      taxPercent,
      totalHours,
      totalDays,
      invoiceValue: invoiceTarget || undefined,
      sectionAComments: sectionAComments || undefined,
      sectionBComments: sectionBComments || undefined,
      wbsDetails: buildWbsDetails(),
    };

    const label = projectName.trim() || "Untitled Draft";

    if (draftId) {
      // Update the existing draft in-place
      dhStore.updateDraft(draftId, payload);
      toast.success("Draft Updated", { description: `"${label}" has been updated.` });
    } else {
      // Create a brand-new draft
      dhStore.addProject({
        ...payload,
        startDate: new Date().toISOString().slice(0, 10),
        endDate: projectIssuedDate || undefined,
      });
      toast.success("Draft Saved", { description: `"${label}" saved as draft — you can continue editing anytime.` });
    }
