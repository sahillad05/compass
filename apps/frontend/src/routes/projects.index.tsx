...
import { LayoutGrid, List, Search, ArrowRight, Calendar, Plus, X, ChevronRight, Check, Trash2, Calculator, FileText, Clock, ChevronDown } from "lucide-react";
...
function ProjectsPage() {
  ...
  // Separate drafts from published projects
  const draftProjects = useMemo(() =>
    projects.filter((p) => p.wbsStatus === "draft"),
    [projects]
  );

  const visible = useMemo(() => {
    return projects.filter((p) => {
      if (p.wbsStatus === "draft") return false;
  ...
  return (
    <AppShell title="Projects" subtitle="Active, archived and all projects across your portfolio">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative" ref={draftsRef}>
            ...
            >
              ...
              <ChevronDown className={cn("h-3 w-3 transition-transform", draftsOpen && "rotate-180")} />
            </button>

            {/* Dropdown panel */}
            {draftsOpen && (
              <div className="absolute right-0 top-full z-50 mt-1.5 w-72 rounded-xl border border-amber-200 bg-white shadow-lg">
                <div className="flex items-center gap-2 border-b border-amber-100 px-4 py-2.5">
                  ...
                  <span className="ml-auto text-[10px] text-muted-foreground">
                    {draftProjects.length} draft{draftProjects.length !== 1 ? "s" : ""}
                  </span>
                </div>
                ...
                {draftProjects.length === 0 ? (
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-amber-50 p-2">
                    {draftProjects.map((p) => {
                      const client = clients.find((c) => c.id === p.clientId);
                      return (
                        <div key={p.id} className="rounded-lg p-3 hover:bg-amber-50 transition-colors">
                          <div className="flex items-start gap-2">
                            <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md bg-gradient-to-br from-amber-400 to-amber-600 text-[10px] font-bold text-white">
                              {client?.logo ?? "?"}
                            </div>
                          ...
                          <div className="mt-2 space-y-0.5">
                            {p.contractType && (
                              <div className="flex items-center justify-between text-[10px]">
                                ...
                                <span className="font-medium text-gray-700">{p.contractType}</span>
                              </div>
                            )}
                            {p.projectType && (
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-muted-foreground">Type</span>
                                <span className="font-medium text-gray-700">{p.projectType}</span>
                              </div>
                            )}
                            {p.currency && p.budget > 0 && (
                              <div className="flex items-center justify-between text-[10px]">
                                <span className="text-muted-foreground">Value</span>
                                <span className="font-medium text-gray-700">{p.currency} {p.budget.toLocaleString()}</span>
                              </div>
                            )}
                            <div className="flex items-center justify-between text-[10px]">
                          ...
                          <div className="mt-2.5 flex items-center justify-between gap-2">
                            <span className="inline-flex rounded-full border border-amber-300 bg-amber-100 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-amber-700">
                              Draft
                            </span>
                            <Link
                              to="/projects/new"
                            ...
                            >
                              Continue editing <ArrowRight className="h-2.5 w-2.5" />
                            </Link>
                          </div>
                        </div>
                      );
      ...
      {/* Card / List view — full width, no sidebar */}
      <div>
        {view === "card" ? (
