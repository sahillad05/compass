...
export const dhStore = {
  ...
  addProject(input: {
    ...
    const wbsAutoId = clientId ? buildWbsId(clientId) : "—";
      ...
      input.wbsDetails.services.forEach((svc: any, si: number) => {
        DEFAULT_SUBTASKS.forEach((sub, ti) => {
          autoTasks.push({
            ...
            assigneeId: "u3",
            dueDate: svc.endDate || input.endDate || today,
            progress: 0,
          });
        });
      });
  ...
  // Update an existing draft project in-place with new form data
  updateDraft(draftId: string, input: {
    ...
    const proj = state.extraProjects.find((p) => p.id === draftId);
    if (!proj) return false;
    if (input.name !== undefined)              proj.name = input.name || "Untitled Draft";
    if (input.clientId !== undefined)          proj.clientId = input.clientId;
    if (input.description !== undefined)       proj.description = input.description;
    if (input.budget !== undefined)            proj.budget = input.budget;
    if (input.wbsDetails !== undefined)        proj.wbsDetails = input.wbsDetails;
    if (input.engagementManager !== undefined) proj.engagementManager = input.engagementManager;
    if (input.salesPerson !== undefined)       proj.salesPerson = input.salesPerson;
    if (input.contractType !== undefined)      proj.contractType = input.contractType;
    if (input.projectType !== undefined)       proj.projectType = input.projectType;
    if (input.projectIssuedDate !== undefined) proj.projectIssuedDate = input.projectIssuedDate;
    if (input.currency !== undefined)          proj.currency = input.currency;
    if (input.taxPercent !== undefined)        proj.taxPercent = input.taxPercent;
    if (input.totalHours !== undefined)        proj.totalHours = input.totalHours;
    if (input.totalDays !== undefined)         proj.totalDays = input.totalDays;
    if (input.invoiceValue !== undefined)      proj.invoiceValue = input.invoiceValue;
    if (input.sectionAComments !== undefined)  proj.sectionAComments = input.sectionAComments;
    if (input.sectionBComments !== undefined)  proj.sectionBComments = input.sectionBComments;
    emit();
    return true;
  },
...
export function getDraft(draftId: string) {
  return state.extraProjects.find((p) => p.id === draftId && p.wbsStatus === "draft") ?? null;
}
